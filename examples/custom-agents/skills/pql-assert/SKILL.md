...
### Direct Lake Residency Assertions

PQL.Assert provides specialized functions for validating Direct Lake column segment residency using `INFO.STORAGETABLECOLUMNSEGMENTS()`. These functions help monitor and validate warming strategies, detect memory pressure, and ensure optimal Direct Lake performance.

> **Direct Lake Only:** These functions require Direct Lake storage mode. They will fail gracefully on Import/DirectQuery models with a "no segments" message.

> **Model Independence:** All Direct Lake functions use STRING parameters for table/column names and work with any Direct Lake semantic model without schema assumptions.

#### Performance Context

Direct Lake query performance is directly tied to column segment residency:
- **Non-resident columns**: First query can take 30+ seconds (transcoding overhead)
- **Resident columns**: Subsequent queries complete in ~1 second (Import-like performance)
- **Memory pressure**: High-cardinality text columns evict first under pressure

#### Column Residency Functions

- `PQL.Assert.DirectLake.Col.ShouldBeResident(testName, tableName, columnName)` - Asserts that ALL segments of a column are fully resident in memory (100% residency). Use for critical filter columns, relationship keys, and frequently-queried dimensions that must be pre-warmed.

- `PQL.Assert.DirectLake.Col.ResidencyRateShouldBeAtLeast(testName, tableName, columnName, minResidencyPct)` - Asserts that at least a minimum percentage (0-100) of column segments are resident. Use for large fact table columns where 100% residency may not be achievable or necessary (e.g., "at least 80% of SalesAmount should be resident").

#### Table Residency Functions

- `PQL.Assert.DirectLake.Tbl.ResidencyRateShouldBeAtLeast(testName, tableName, minResidencyPct)` - Asserts that at least a minimum percentage (0-100) of ALL segments across ALL columns in a table are resident. Use for table-level warming strategy validation (e.g., "Customer dimension should be 90%+ resident", "Sales fact should maintain 40%+ residency").

#### Use Cases

**Pre-warming Validation:**
```dax
// Verify warming script succeeded
EVALUATE PQL.Assert.DirectLake.Col.ShouldBeResident(
    "Product name must be fully resident after warming",
    "Product",
    "ProductName"
)
```
...
```dax
// Monitor fact table residency
EVALUATE PQL.Assert.DirectLake.Tbl.ResidencyRateShouldBeAtLeast(
    "Sales table should maintain 50%+ residency",
    "Sales",
    50
)
```
...
```dax
// Alert if critical columns drop below threshold
EVALUATE PQL.Assert.DirectLake.Col.ResidencyRateShouldBeAtLeast(
    "ProductKey residency dropped below 80% - possible eviction",
    "Sales",
    "ProductKey",
    80
)
...
**Warming Strategy Design:**
```dax
// Test warming targets across environment tiers
// DEV: Test with low thresholds
// TEST: Validate production-like thresholds
// PROD: Continuous monitoring with alerts
```
